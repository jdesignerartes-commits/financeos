...
export function DashboardFilters({
  ...
  const accountItems = useMemo(
    () => [{ value: ALL, label: "Todas as contas" }, ...accounts.map((a) => ({ value: a.id, label: a.name }))],
    [accounts],
  );
  const creditCardItems = useMemo(
    () => [{ value: ALL, label: "Todos os cartões" }, ...creditCards.map((c) => ({ value: c.id, label: c.name }))],
    [creditCards],
  );
  const categoryItems = useMemo(
    () => [{ value: ALL, label: "Todas as categorias" }, ...categories.map((c) => ({ value: c.id, label: c.name }))],
    [categories],
  );
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {searchParams.toString() && (
        <Button type="button" variant="ghost" size="sm" onClick={() => router.push("/overview")}>
          Limpar filtros
        </Button>
      )}
    </div>
